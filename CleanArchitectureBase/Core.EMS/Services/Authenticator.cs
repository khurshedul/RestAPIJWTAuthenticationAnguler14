...
namespace Core.EMS.Services
{
    ...
    {
        Task<BaseResponse> AuthenticateAsync(string username, string password);
    }
    ...
    public class JwtAuthenticationManager : IJwtAuthenticationManager
    {
        ...
        public JwtAuthenticationManager(IUserService userInformationService)
        {
            _userInformationService = userInformationService;
        }

        public async Task<BaseResponse> AuthenticateAsync(string username, string password)
        {
            var loginResult = await _userInformationService.Login(username, password, "", "");

            if (!loginResult.IsSuccessful)
            {
                return new BaseResponse { IsSuccessful = false, Message = "Unauthorized" };
            }
            ...
            var siginingCredentials = new SigningCredentials(securityKey, algorithm);

            var token = new JwtSecurityToken(Constants.Issuer, Constants.Audiance, claims, notBefore: DateTime.Now, expires: DateTime.Now.AddHours(1), siginingCredentials);

            var tokenJson = new JwtSecurityTokenHandler().WriteToken(token);
            var loginInfo = (LoginInformation)loginResult.Data;
            loginInfo.AccessToken = tokenJson;

            return new BaseResponse { IsSuccessful = true, Data = loginInfo };
        }
    }
    ...
    public static class Constants
    {
        ...
        public const string Audiance = "https://localhost:44325/";
        public const string Secret = "myAppSecretforapidevelopmentKhurshedulBaridSayem";
    }
}
